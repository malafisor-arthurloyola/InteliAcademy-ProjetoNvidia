...
import { startups, NVIDIA_TECHS, SOURCE_TYPES, type AIMaturity, type NvidiaTech, type SourceType } from "@/lib/mock-data";
...
function Ranking() {
  ...
  const filtered = useMemo(() => {
    return startups.filter((s) => {
      ...
      if (s.foundedYear < yearRange[0] || s.foundedYear > yearRange[1]) return false;
      ...
      if (techs.length && !s.recommendations.some((r) => techs.includes(r.tech))) return false;
      if (sourceTypes.length && !s.evidences.some((e) => sourceTypes.includes(e.type))) return false;
      return true;
    });
        ...
        <ScrollArea className="h-[70vh] pr-3">
          <div className="space-y-4">
            <FilterGroup title="Maturidade de IA">
              {MATURITIES.map((m) => <CheckRow key={m} value={m} set={setMaturity} current={maturity} />)}
            </FilterGroup>
            <FilterGroup title="Setor">
              {SECTORS.map((s) => <CheckRow key={s} value={s} set={setSectors} current={sectors} />)}
            </FilterGroup>
            <FilterGroup title="Região">
              {REGIONS.map((r) => <CheckRow key={r} value={r} set={setRegions} current={regions} />)}
            </FilterGroup>
            <FilterGroup title={`Ano de fundação: ${yearRange[0]}–${yearRange[1]}`}>
              <Slider min={2015} max={2026} step={1} value={yearRange} onValueChange={(v) => setYearRange([v[0], v[1]] as [number, number])} />
            </FilterGroup>
            <FilterGroup title="Valuation / faixa">
              <p className="text-[11px] text-muted-foreground">US$ 10M – 100M (mock)</p>
              <Slider defaultValue={[10, 100]} min={0} max={200} step={5} />
            </FilterGroup>
            <FilterGroup title="Funding / faixa">
              <p className="text-[11px] text-muted-foreground">US$ 1M – 30M (mock)</p>
              <Slider defaultValue={[1, 30]} min={0} max={50} step={1} />
            </FilterGroup>
            <FilterGroup title={`Potencial de crescimento ≥ ${minGrowth}`}>
              <Slider value={[minGrowth]} onValueChange={(v) => setMinGrowth(v[0])} max={100} step={5} />
            </FilterGroup>
            <FilterGroup title={`Prob. de aceitar contato ≥ ${minContact}`}>
              <Slider value={[minContact]} onValueChange={(v) => setMinContact(v[0])} max={100} step={5} />
            </FilterGroup>
            <FilterGroup title={`Score de evidência ≥ ${minEvidence}`}>
              <Slider value={[minEvidence]} onValueChange={(v) => setMinEvidence(v[0])} max={100} step={5} />
            </FilterGroup>
            <FilterGroup title="Tipo de fonte">
              {SOURCE_TYPES.map((t) => <CheckRow key={t} value={t} set={setSourceTypes} current={sourceTypes} />)}
            </FilterGroup>
            <FilterGroup title="Tecnologia NVIDIA recomendada">
              {NVIDIA_TECHS.map((t) => <CheckRow key={t} value={t} set={setTechs} current={techs} />)}
            </FilterGroup>
          </div>
        </ScrollArea>
      </Card>
        ...
        <Card className="p-3">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:flex sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="text-[11px]">{filtered.length} de {startups.length}</Badge>
            </div>
          </div>
        </Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] border-collapse text-sm">
              ...
              <tbody>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="truncate text-[11px] text-muted-foreground">{s.fundingRange}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-foreground">{s.sector}</td>
                    <td className="px-3 py-2.5 text-xs text-foreground">{s.region}</td>
                    <td className="px-3 py-2.5 text-xs tabular-nums text-foreground">{s.foundedYear}</td>
                    <td className="px-3 py-2.5"><MaturityBadge value={s.maturity} /></td>
                    <td className="px-3 py-2.5"><ContactStatusBadge status={status} /></td>
                    <td className="px-3 py-2.5 w-40"><ScoreBar value={s.radarScore} /></td>
                    <td className="px-3 py-2.5 w-40"><ScoreBar value={s.evidenceConfidence} /></td>
                    <td className="px-3 py-2.5 w-40"><ScoreBar value={s.growthPotential} /></td>
                    <td className="px-3 py-2.5 w-40"><ScoreBar value={s.nvidiaFit} /></td>
                    <td className="px-3 py-2.5 w-40"><ScoreBar value={s.contactPriority} /></td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={12} className="px-3 py-10 text-center text-xs text-muted-foreground">Nenhuma startup com os filtros atuais.</td></tr>
                )}
              </tbody>
            </table>
          </div>
